module Main where

import System.Exit (exitSuccess, exitFailure)
import Control.Monad.State.Lazy

import SReplay

-- | Runs the test suite for the replay library
main :: IO ()
main = do
  results <- runTests
  if and results
    then exitSuccess
    else exitFailure

-- | state attribute, count the number of 'iot' action a ReplayT monad has
type IOCount = Int

-- | the state monad, constructed by 'State' from 'Control.Monad.State.Lazy', includes 'IOCount' as the inner state
type StateMonadCount = State IOCount

-- | a function that takes a 'StateMonadCount' monad and returns a 'ReplayT' monad
type Program = StateMonadCount () -> ReplayT StateMonadCount () Int Int

-- | A result is a pair of the final result of the program
--   and the number of 'ticks' executed.
type Result  = (Int, Int)
type Input   = [Int]

-- | A test case collects a program and a list of answers together
--   with its expected result.
data TestCase = TestCase
  { testName    :: String
  , testInput   :: Input
  , testResult  :: Result
  , testProgram :: Program
  }

runProgram :: Program -> Input -> Result
runProgram p inp = runState sm 0
  where sm = play (p addCount) emptyTrace inp
        addCount = modify (+1)
        play prog t inp = do
          r <- runt prog t
          case r of
            Right x -> case inp of
                         [] -> return x
                         _  -> error "too many inputs"
            Left (_, t') -> case inp of
                              [] -> error "too few inputs"
                              a : inp' -> play prog (addAnswer t' a) inp'
          

-- | Checking a test case. Compares expected and actual results.
checkTestCase :: TestCase -> IO Bool
checkTestCase (TestCase name i r p) = do
  putStr $ name ++ ": "
  let r' = runProgram p i
  if r == r'
    then putStrLn "ok" >> return True
    else putStrLn ("FAIL: expected " ++ show r ++
                  " instead of " ++ show r')
         >> return False


-- | List of interesting test cases.
testCases :: [TestCase]
testCases =
  [ TestCase
    { testName    = "example"
    , testInput   = [59,65]
    , testResult  = (2^59 * 3^65, 6)
    , testProgram = \tick -> do
        iot tick
        iot tick
        age  <- askt ()
        iot tick       
        name <- askt () 
        iot tick        
        iot tick        
        iot tick
        return (2^age * 3^name)
    }
  , TestCase
    { testName    = "test1"
    , testInput   = [3,4]
    , testResult  = (8, 1)
    , testProgram = \tick -> do
        iot tick
        a <- askt () -- should be 3
        b <- iot (return 1)
        c <- askt () -- should be 4
        return (a + b + c)
    }
  ]

-- | Running all the test cases.
runTests :: IO [Bool]
runTests = mapM checkTestCase testCases
